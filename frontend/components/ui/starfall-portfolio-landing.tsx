...
const AuroraBackground: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const observer = new MutationObserver(() => {
          ...
          renderer.domElement.style.mixBlendMode = dark ? 'normal' : 'multiply';
          renderer.domElement.style.opacity = dark ? '1' : '0.3';
        });
        observer.observe(document.documentElement, { attributes: true, attributeFilter: ['class'] });
        animate();
        return () => { observer.disconnect(); cancelAnimationFrame(animationFrameId); window.removeEventListener('resize', handleResize); if (currentMount.contains(renderer.domElement)) currentMount.removeChild(renderer.domElement); renderer.dispose(); material.dispose(); geometry.dispose(); };
    }, []);
    return <div ref={mountRef} />;
};

// --- DEFAULT DATA ---
const defaultData: Required<Omit<PortfolioPageProps, 'showAnimatedBackground' | 'navExtra' | 'children'>> = {
            ...
            <div className="max-w-7xl mx-auto flex justify-between items-center">
                <div className="flex items-center space-x-2">
                    <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center">
                        <span className="geist-font text-sm font-bold text-primary-foreground">{logo.initials}</span>
                    </div>
                ...
                <div className="flex items-center gap-3">
                    {navExtra}
                    <button onClick={resume?.onClick} className="text-muted-foreground hover:text-foreground transition-colors text-sm font-medium cursor-pointer">{resume?.label}</button>
                </div>
            </div>
        </nav>
        <div className="divider" />
        <main id="hero" className="w-full min-h-screen flex flex-col items-center justify-center px-6 py-20">
            <div className="max-w-6xl mx-auto text-center">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
                    <button onClick={ctaButtons.primary.onClick} className="text-muted-foreground hover:text-foreground transition-colors text-sm font-medium cursor-pointer">{ctaButtons.primary.label}</button>
                    <button onClick={ctaButtons.secondary.onClick} className="text-muted-foreground hover:text-foreground transition-colors text-sm font-medium cursor-pointer">{ctaButtons.secondary.label}</button>
                </div>
